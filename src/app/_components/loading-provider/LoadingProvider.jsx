...
export const LoadingProvider = ({ children, minimumLoadingTime = 3000, showProgress = true }) => {
  const [isLoading, setIsLoading] = useState(false);
  const router = useRouter();
  const pathname = usePathname();
  const loadingStartTime = useRef(null);
  ...
  const navigateWithLoader = useCallback((url) => {
    // Don't show loader if we're already on this page or navigation is in progress
    if (currentPathname.current === url || navigationInProgress.current) return;
  ...
  // Update current pathname ref when pathname changes
  useEffect(() => {
  ...
  // Hide loading when pathname changes (navigation completed)
  useEffect(() => {
    if (navigationInProgress.current && pathname !== currentPathname.current) {
  ...
  }, [pathname, isLoading, minimumLoadingTime]);
