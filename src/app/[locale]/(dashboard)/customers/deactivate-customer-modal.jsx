...
export default function DeactivateCustomerModal({
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteAdmin(customer.id);
      toast.success(t("customer-deactivated-successfully"));
      onCustomerDeactivate?.(); // Refresh table
      onClose();
    } catch (error) {
      console.error(error);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[400px]">
        ...
        <div className="text-center text-md text-gray-600">
          {t("delete-confirmation")} <strong>{customer?.name}</strong>?
        </div>
        ...
        <DialogFooter className="pt-4 flex gap-2">
          ...
          >
            {isDeleting ? <MoonLoader size={18} color="#fff" /> : t("deactivate")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
